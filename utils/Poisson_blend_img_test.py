...
def Poisson_blend_img(imgTrg, imgSrc_gx, imgSrc_gy, holeMask, gradientMask=None, edge=None):
    ...
    # prepare discrete Poisson equation 泊松方程 https://zhuanlan.zhihu.com/p/68349210
    # 同时参考前景图片和背景图片，可以直接等于前景像素的散度，也可以在前景和背景在同一点像素的散度进行某种组合得到
    # 保持边界一致，边界条件上像素值等于背景图片即可
    ...
    for ch in range(nCh):
        ...
        # solve Poisson equation lsqr：最小二乘法
...
# 求解ROI的梯度场Isrc，及背景图像不被修改的像素区域的梯度场Idst。然后通过Isrc+Idst得到整幅待重建图像的梯度场，最后才根据梯度场求解散度。所以千万不要偷懒，不要一步求解散度，要先把待重建图像的梯度场求好，再进行求解散度。 https://blog.csdn.net/hjimce/article/details/45716603
